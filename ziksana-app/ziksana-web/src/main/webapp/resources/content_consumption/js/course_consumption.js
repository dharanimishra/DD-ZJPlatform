  ...
  function add_learner_content_note(note_title, note_description, note_duration){
	  ...
	  $.post('/ziksana-web/secure/addLearnerNote', {"courseId":course_id, 'nodeId':node_id, "noteTitle":note_title, "noteDescription":note_description, "noteDuration":note_duration}, function(data){
		  ...
		  if(data == '1'){
				...
				jwplayer(reftoplayer).play(true); //resume play
	  ...
	  $.get('/ziksana-web/secure/getLearnerNotes', {"courseId":course_id, "nodeId":node_id}, function(data){
		  ...
		  var links = '';
			...
			for (i in notes)
			{
			    ...
			    link = '<a data-iconprefix="note" title="'+description+'" onclick="jwplayer(reftoplayer).seek('+duration+').play(true);">'+title+'</a>';
			
			    links += link;
				ff_note_data=title+"||"+description+"||"+duration+"|||";
			}
	  ...
	  $.get('/ziksana-web/secure/getLearnerQuestions', {"courseId":course_id, "nodeId":node_id}, function(data){
		  ...
		  var links = '';
			...
			for (i in questions)
			{
			    ...
			    link = '<a style="display:block; margin: .5em 0;" data-iconprefix="question"  onclick="jwplayer(reftoplayer).seek('+duration+').play(true);">'+title+'</a>';
			
			    links += link;
				ff_question_data=title+"||"+duration+"|||";
			}
	  ...
	  $.get('/ziksana-web/secure/getContentTOC', {"courseId":course_id, "nodeId":node_id}, function(data){
		  ...
		  var links = '';
			...
			for (i in toc_elements)
			{
			    ...
			    link = '<a style="display:block; margin: .5em 0;" data-iconprefix=""  onclick="jwplayer(reftoplayer).seek('+duration+').play(true);">'+title+'</a>';
