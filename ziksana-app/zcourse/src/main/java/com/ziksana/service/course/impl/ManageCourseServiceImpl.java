...
public class ManageCourseServiceImpl implements ManageCourseService {
	...
	private static Logger logger = Logger.getLogger(ManageCourseServiceImpl.class);
	...
	public void deleteEngagementRule(Engagement engagement)
		...
		engagementMapper.deleteCriteria(isDelete, new Integer(criteria.getEngagementCriteriaId().getStorageID()));
		
		engagementMapper.deleteEngagement(isDelete, new Integer(engagement.getEngagementId().getStorageID()));

	}

	@Transactional
	@Override
	public List<MemberSubscriptionProgram> getEnrolledStudentsInfo(Integer curriculumCourseId)
			throws CourseException {
		...
		progCurriculumCourseId = new Integer(currCourse.getProgramCurriculum().getProgramCurriculumId().getStorageID());
		
		learningProgram = programsMapper.getLearningProgramByProgCurriculum(progCurriculumCourseId);
		
		memberProgList = sbnCourseMapper.getMemberSubscriptionPrograms(new Integer(learningProgram.getLearningProgramId().getStorageID()));
	...
	public SubscriptionCourse getCoursePerformance(Integer curriculumCourseId)
		...
		sbnCourse = sbnCourseMapper.getSubscribedStudentsInfo(curriculumCourseId);
	...
	public List<GroupMember> getStudentGroupPerformance(Integer memberRoleId)
		...
		groupMemberList = groupsMapper.getGroupMemberByMemberRoleId(memberRoleId);
	...
	public void associateStudentToGroup(Integer groupId, GroupMember groupMember)
		...
		if(group.getGroupId() == null){
			throw new CourseException("Group ID cannot be null to Associate to Group");
		}
		
		logger.debug("Group ID : "+groupId);
		
		if(groupMember.getMemberRole() == null){
			throw new CourseException("Member Role for Group Member cannot be null to Associate to Group");
		}
	...
	public List<GroupMemberConversation> getStudentInteraction(
		...
		for (GroupMember groupMember : groupMemberList) {
			...
			sourceGroupMemberId = new Integer(groupMember.getGroupMemberId().getStorageID());
	...
	public void removeInteraction(Integer conversationId)
		...
		if(conversationId == null){
			throw new CourseException("Student/Learner's Interaction ID cannot be null");
		}
	...
	public void removeStudentFromGroup(Integer groupId, Integer groupMemberId)
		...
		if(groupId == null){
			throw new CourseException("Student/Learner's Group ID cannot be null");
		}

		if(groupMemberId == null){
			throw new CourseException("Student/Learner's GroupMember ID cannot be null");
		}
		
		logger.debug("Group ID : ["+groupId +"]  , Group Member ID : ["+groupMemberId+"]");
	...
	public List<Group> getGroupsByCourse(Integer courseId,
		...
		if(courseId == null || curriculumCourseId == null){
			throw new CourseException("CourseID/CurriculumCourseID cannot be null");
		}
	...
	public void createGroupForCourse(Group group) throws CourseException {
		...
		if(context.getCourse() == null || context.getCurriculumCourse() == null){
			throw new CourseException("Course/CurriculumCourse cannot be null to create a new Group");
		}
	...
	public void deleteGroup(Integer groupId) throws CourseException {
		...
		//TODO: Send a Message 	Notification to Members with in then Group for activities/submissions
	...
	public List<GroupMemberActivity> getGroupActivities(Integer groupId)
		...
		for (GroupMember groupMember : groupMembersList) {
			...
			groupActivitiesList = groupsMapper.getGroupActivities(new Integer(groupMember.getGroupMemberId().getStorageID()));
