...
public interface TreeNodeMapper {
	...
	@Select({ "select cclc.CourseId as courseid,course.CourseDuration as duration,c.ID as learningcomponentid, c.Name as modulename,nest.ParentLearningComponentId as ParentLearningComponentId,nest.NestLearningComponentId as NestLearningComponentId,c.Description AS Description, c.ThumbnailPicturePath as ThumbnailPicturePath  from corcourse course, corcourselearningcomponent cclc, corlearningcomponent c,corlearningcomponentnest nest where cclc.LearningComponentId=c.ID and  cclc.courseid=#{courseId,jdbcType=INTEGER} and c.isdelete=false and course.ID= cclc.courseid and course.IsDelete=false and c.ID=nest.NestLearningComponentId" })
	@Results(value = {
			...
			@Result(property = "duration", column = "duration"),
			@Result(property = "nodeDescription", column = "Description"),
